...
export class RateMyProfessor {
    collegeName;
    teacherName;
    constructor(college_name, teacher_name) {
    ...
    // get college summary
    async get_college_info(retrieve_all) {
        if (retrieve_all) {
            return await search_school(this.collegeName);
        }
        else {
            return await filter_school(await search_school(this.collegeName), this.collegeName);
        }
    }
    async get_college_info_and_save(file_name, retrieve_all) {
    ...
    async get_professor_list_and_save(file_name) {
    ...
    set_college(new_college_name) {
        this.collegeName = new_college_name;
    }
    set_professor_name(new_professor_name) {
        this.teacherName = new_professor_name;
    }
    set_college_and_professor_name(college_name, professor_name) {
        this.set_college(college_name);
        this.set_professor_name(professor_name);
    }
    async get_comments_by_professor() {
        if (!this.teacherName) {
            console.error("Name of professor is empty, please add professor name before continuing!");
        }
        else {
            return await get_professor_rating(this.teacherName, this.collegeName);
        }
    }
    async get_comments_by_professor_and_save(file_name) {
    ...
    async get_professor_info_and_save(file_name) {
