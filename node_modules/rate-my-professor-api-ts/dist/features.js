...
const TEACHER_COMMENTS = `\"query TeacherRatingsPageQuery($id: ID!) {
        node(id: $id) {
            __typename
            ... on Teacher {
                firstName
...
const GET_TEACHER_ID_QUERY = `\"query TeacherSearchResultsPageQuery(
        $query: TeacherSearchQuery!
        $schoolID: ID
        $includeSchoolFilter: Boolean!
    ) {
...
//   isSaved
// }"`;
const TEACHER_LIST_QUERY = `\"query TeacherSearchResultsPageQuery(
        $query: TeacherSearchQuery!
        $schoolID: ID
        $includeSchoolFilter: Boolean!
    ) {
...
async function search_school(schoolName) {
    ...
    for (const data of school_search_array) {
        ...
        college_to_department_map.set(data["node"]["name"], data["node"]["departments"]);
...
async function filter_school(raw_school_data, original_school_name) {
    const filtered_school_data = [];
    const filtered_department_map = new Map();
...
async function retrieve_school_id(school_name) {
    let school_id = (await search_school(school_name)).school_node[0]["id"];
    return school_id;
}
async function search_teacher(professor_name, school_name) {
    try {
        // retrieve school id
...
async function get_professor_rating(professor_name, college_name) {
    try {
...
async function get_professor_list_by_school(college_name) {
